...
func extractPackageInfo(snapshots *config.Snapshots, arch config.Arch, distro config.Distro, packages map[string]bool) (map[string]*deb.Package, []string, error) {
	...
	// Test if this can be extended without completely rewritting the program.
	pkgGrp = append(pkgGrp, &deb.PackageIndex{
		URL:      "http://apt.postgresql.org/pub/repos/apt/dists/bionic-pgdg/main/binary-amd64/Packages.gz",
		Arch:     config.AMD64,
		PoolRoot: "http://apt.postgresql.org/pub/repos/apt/",
		Distro:   config.BIONIC_PGDG,
		Channel:  "main",
	})
	pkgGrp = append(pkgGrp, &deb.PackageIndex{
		URL:      "http://apt.postgresql.org/pub/repos/apt/dists/bionic-pgdg/main/binary-arm64/Packages.gz",
		Arch:     config.ARM64,
		PoolRoot: "http://apt.postgresql.org/pub/repos/apt/",
		Distro:   config.BIONIC_PGDG,
		Channel:  "main",
	...
	for _, pix := range pkgGrp {
		// fmt.Println("DEBUG:", pix.URL, pix.Arch, pix.PoolRoot, pix.Distro, pix.Snapshot)
